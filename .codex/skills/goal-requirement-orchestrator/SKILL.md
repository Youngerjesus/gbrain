...
9. If the current slice is UI-bearing, run `plan-design-review` after the Draft Plan is created and before `plan-eng-review`, using the current requirement, research/design gate state, Draft Plan, and existing design/code patterns as inputs. This gate must review information architecture, interaction states, user journey, design specificity, design-system alignment, responsive/accessibility coverage, and unresolved UI decisions. Record the review outcome, locked design decisions, deferred items, and any blockers in the current requirement's `progress.md`, `decisions.md`, and `evidence.md`. If the slice is not UI-bearing, record `Gate status: not_required` with the reason before `plan-eng-review`.
10. If the current slice is user-facing experience work, run `plan-ux-review` after the Draft Plan is created and before `plan-eng-review`, using the current requirement, research/design gate state, Draft Plan, existing UX patterns, and target user/job evidence as inputs. This gate must review first value, core task flow, information architecture, interaction states, recovery/re-entry, trust/clarity, accessibility, and UX measurement. Record the review outcome, locked UX decisions, deferred items, blockers, and whether the Draft Plan needs reconciliation before `plan-eng-review`. If the slice is not user-facing experience work, record `Gate status: not_required` with the reason before `plan-eng-review`.
11. If the current slice is developer-facing experience work, run `plan-devex-review` after the Draft Plan is created and before `plan-eng-review`, using the current requirement, research/design gate state, Draft Plan, existing docs/API/CLI/SDK patterns, and target developer persona evidence as inputs. This gate must review time to hello world, getting started, API/CLI/SDK ergonomics, error quality, documentation, upgrade path, developer tooling, ecosystem, and DX measurement. Record the review outcome, locked DX decisions, deferred items, blockers, and whether the Draft Plan needs reconciliation before `plan-eng-review`. If the slice is not developer-facing experience work, record `Gate status: not_required` with the reason before `plan-eng-review`.
12. For this requirement, run the remaining pre-implementation review gates in order: `plan-eng-review`, then `scenario-brake`, using the current requirement, research/design gate state, any `plan-design-review`, `plan-ux-review`, or `plan-devex-review` outcome, and Draft Plan as inputs. `plan-eng-review` and `scenario-brake` must invoke their relevant read-only companion subagents according to each skill's companion routing contract when the runtime allows subagents. If delegation is unavailable, record the explicit fallback or blocker in the current requirement's `progress.md`.
13. Run `secondary-plan` to reconcile review findings from `plan-design-review`, `plan-ux-review`, `plan-devex-review`, `plan-eng-review`, and `scenario-brake`; update `plans/<plan-id>/plan.md`, and write `plans/<plan-id>/secondary_plan.md`. Implementation must follow those accepted plan artifacts, not the earlier draft alone.
...
- After `plan-design-review`, record the UI scope classification, gate status, review outcome, locked design decisions, blockers, and whether the Draft Plan needs reconciliation before `plan-eng-review`.
- After `plan-ux-review`, record the user-facing experience classification, gate status, review outcome, locked UX decisions, blockers, and whether the Draft Plan needs reconciliation before `plan-eng-review`.
- After `plan-devex-review`, record the developer-facing experience classification, gate status, review outcome, locked DX decisions, blockers, and whether the Draft Plan needs reconciliation before `plan-eng-review`.
...
For `plan-eng-review`, `scenario-brake`, `visual-qa-hardening`, and `implementation-brake`, unavailable companion subagent delegation is also a gate-level fallback or blocker that must be recorded before continuing.
...
1. Identify the smallest sequence of requirement document paths needed for the broad initiative.
2. Add a concise overall goal, sequence rationale, and `Sequence Outcome` so later goal runs understand how the requirements connect and what must exist after all listed requirements are complete.
3. For each listed requirement, include a short slice outcome in the sequence checklist. This is an ordering and handoff note, not full acceptance criteria.
4. Create or update `goal-requirements/<id>/sequence.md` and `goal-requirements/<id>/progress.md`.
5. Create or update the first listed `requirements/<requirement-id>/requirements.md` with `requirement-clarifier`; create its `progress.md`, `decisions.md`, and `evidence.md` state files as needed before starting the requirement.
6. Ensure the first requirement's `requirement-clarifier` run invokes `requirement-clarifier-post-draft-reviewer` when the runtime allows subagents. Record `reviewer_status`, any fallback status, and unresolved findings in the first requirement's state files before treating it as accepted.
7. Do not write or rewrite later `requirements/<requirement-id>/requirements.md` files unless the user explicitly asks to start that stage or the later stage document is reached, missing, stale, or not accepted.
8. Add the fixed execution gates so later goal runs know the required research, design, pre-implementation, implementation, post-implementation reviews, and any final production readiness gate.
9. In the final response, tell the user the first requirement path, its reviewer-backed acceptance status or blocker, and that implementation still must start at state rehydration plus step 1 of the `Execution Gates`, not direct implementation.
