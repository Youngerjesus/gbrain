...
description: Split a broad initiative into ordered requirement-clarifier outputs, then execute each slice through requirement-clarifier, research, technical-design, conditional plan-design-review, plan-ux-review, plan-devex-review, plan-eng-review, scenario-brake, secondary-plan, implementation, conditional ux-review/devex-review, closeout, and any required final production-readiness gate.
---
...
This skill bootstraps only the first requirement, not every requirement. It creates a minimal orchestration file that tells the next Codex goal which `requirements/<requirement-id>/requirements.md` files to create or execute, in what order, what outcome should exist when the sequence is fully complete, what outcome each later slice should contribute, which slices are complete, which conditional hard gates must be passed before moving to the next slice, and whether the sequence needs a final production readiness launch gate. The first listed requirement is created or updated immediately with `requirement-clarifier`, including its post-draft reviewer gate; later listed requirements are not written until their slice starts.

## Contract

This skill creates and advances one requirement at a time. A sequence path is not direct implementation permission: every slice must rehydrate state files, pass the required gates, record evidence, and complete closeout before the next item starts.

The orchestrator owns ordering, state files, Worktree Execution Policy, Delegated Subagent Lifecycle, and completion bookkeeping. It must resist becoming a giant state machine for implementation details; specialized skills own requirement clarification, design, planning, review, implementation, and closeout.

Before creating or resuming a sequence, classify the user's requested artifact class and the checklist's execution unit separately. Do not let a goal id, sequence title, or domain phrase such as "reference", "IA", "design", or a product name override the actual requested artifact. If the user asks to organize reference material, extract a clone-coding contract, create a design/input contract, or produce another prerequisite artifact, that artifact is its own goal-requirements sequence unless the active sequence's checklist items are also about producing that same artifact. Do not bury a distinct artifact-class request as a prerequisite paragraph inside an implementation sequence for screens, routes, modules, or data changes.
...
requirements/<first-requirement-id>/requirements.md
requirements/<first-requirement-id>/progress.md
requirements/<first-requirement-id>/decisions.md
requirements/<first-requirement-id>/evidence.md
```

Use a short kebab-case `<id>` from the initiative name unless the user provides one. Treat `goal-requirements/` as local planning state.
When creating a new generated goal id, inspect the existing sibling directories under `goal-requirements/` and prefix the id with the next three-digit sequence number, starting at `001`. Use the highest existing leading three-digit numeric prefix plus one, such as `001-editor-rewrite`, `002-editor-launch`, then `003-editor-readiness`. Ignore sibling directories without that prefix when calculating the next number. If the user provides an exact goal id or path, preserve it as given. Do not renumber existing goal requirement directories.
When listing new generated requirement ids inside the sequence, apply the same next-three-digit-prefix rule against existing sibling directories under `requirements/`, preserving any exact requirement ids or paths supplied by the user.
Create only the first requirement document during orchestration bootstrap. Later requirement documents stay as planned paths plus slice outcomes in `sequence.md` until their slice starts.

## File Shape

Keep `sequence.md` small. Include only the overall goal, how the requirement sequence connects, the sequence outcome, the requirement paths with short slice outcomes, completion checkboxes, state-file rules, blocked rules, completion contract, context budget rules, the fixed execution gates, and the final production readiness rule when the sequence is MVP, beta, launch, or production-bound unless the user explicitly asks for more.
...
If a dependency is essential to avoid executing the wrong next slice, add it inline and keep it brief:

```markdown
- [ ] 2. `requirements/<requirement-id-2>/requirements.md` - after item 1 is planned, passed any required `plan-design-review`, implemented, verified, passed any required `visual-qa-hardening`, passed `implementation-brake`, completed `closeout`, and checked complete
```

## Workflow

1. Run the sequence boundary check: classify the requested artifact class, the execution unit of each checklist item, and whether the request belongs in the current sequence or needs a separate sequence. If the requested artifact class differs from the current sequence's execution unit, or if satisfying the request would add a new requirement path to an existing sequence, ask the user for explicit approval before changing that sequence's scope; otherwise create or select a separate sequence.
2. Identify the smallest sequence of requirement document paths needed for the broad initiative.
3. Add a concise overall goal, sequence rationale, and `Sequence Outcome` so later goal runs understand how the requirements connect and what must exist after all listed requirements are complete.
4. For each listed requirement, include a short slice outcome in the sequence checklist. This is an ordering and handoff note, not full acceptance criteria.
5. Create or update `goal-requirements/<id>/sequence.md` and `goal-requirements/<id>/progress.md`.
6. Create or update the first listed `requirements/<requirement-id>/requirements.md` with `requirement-clarifier`; create its `progress.md`, `decisions.md`, and `evidence.md` state files as needed before starting the requirement.
7. Ensure the first requirement's `requirement-clarifier` run invokes `requirement-clarifier-post-draft-reviewer` when the runtime allows subagents. Record `reviewer_status`, any fallback status, and unresolved findings in the first requirement's state files before treating it as accepted.
8. Do not write or rewrite later `requirements/<requirement-id>/requirements.md` files unless the user explicitly asks to start that stage or the later stage document is reached, missing, stale, or not accepted.
9. Add the fixed execution gates so later goal runs know the required research, design, pre-implementation, implementation, post-implementation reviews, and any final production readiness gate.
10. In the final response, tell the user the first requirement path, its reviewer-backed acceptance status or blocker, and that implementation still must start at state rehydration plus step 1 of the `Execution Gates`, not direct implementation.

## Constraints

- Do not create a combined requirements document.
- Do not mix different artifact classes in one sequence. Reference-material organization, clone-coding contract extraction, design/input contract creation, screen implementation, route implementation, data migration, and launch readiness are separate sequence candidates unless the user explicitly asks to combine them and the sequence outcome names the combined artifact.
- Do not insert a new requirement into an existing sequence because it is adjacent, prerequisite-like, or related to the sequence name. Inserting or rewriting requirement paths in an existing sequence requires explicit user approval unless the user directly asked to modify that sequence's scope.
- For MVP, beta, launch, or production-bound initiatives, include a final production readiness requirement path such as `requirements/<goal-id>-production-readiness/requirements.md`. The file may be created later with `requirement-clarifier` when that slice is reached.
- Do include the sequence-level outcome and short per-slice outcomes in `sequence.md`.
- Do not include full acceptance criteria, technical plans, scenario matrices, or implementation details in `sequence.md`.
- Keep the `Goal` and `Sequence Rationale` sections short; they explain connection and order, not implementation.
- Use `goal-requirements/`, not the typo path `goal-requirments/`.
- Create sequence-level `progress.md`; create the first requirement's `requirements.md` with `requirement-clarifier` during orchestration bootstrap, and create requirement-level `progress.md`, `decisions.md`, and `evidence.md` before starting each requirement.
...
- Read the authoritative state files and git status/diff at the start of every work slice; worktree state, accepted artifacts, and evidence override prior conversation memory.
- Update state at gate boundaries, not only at closeout.
- Do not duplicate `requirement-clarifier`, `secondary-plan`, `plan-design-review`, `plan-ux-review`, `plan-devex-review`, `plan-eng-review`, `scenario-brake`, `ux-review`, `devex-review`, `implementation-brake`, or `closeout`.
- Do not duplicate `research` or `technical-design`; invoke those skills when the conditional hard gates are required.
- The first requirement is special only during orchestration bootstrap: create it with `requirement-clarifier`, run its post-draft reviewer gate, and record its acceptance state. After that, it must pass the same pre-coding gates as every later requirement.
- A listed requirement path or an existing requirement document is not enough to start coding; each requirement must evaluate `research`, evaluate `technical-design`, produce a requirement-based Draft Plan artifact with `Status: draft`, then pass any required UI-bearing `plan-design-review`, user-facing `plan-ux-review`, developer-facing `plan-devex-review`, `plan-eng-review`, `scenario-brake`, and `secondary-plan` gates first.
- `plan-eng-review`, `scenario-brake`, `visual-qa-hardening`, and `implementation-brake` must use their relevant read-only companion subagents according to each skill's companion routing contract when the runtime allows subagents. If companion delegation cannot run, record the explicit fallback or blocker before continuing.
- For UI-bearing requirements, `visual-qa-hardening` is conditional but mandatory once triggered by user-facing UI, layout, visual state, responsive behavior, or reference-driven visual acceptance criteria. It runs after browser screenshot verification and before `implementation-brake`.
- Apply root `AGENTS.md` context-loading rules before coding; if triggered, `context-loading` must run before `tdd-workflow`.
- Do not mark a requirement checkbox complete when only clarification, planning, coding, tests, visual QA, experience review, or `implementation-brake` are done; completion requires `implementation-brake` `[SHIP]`, `closeout`, closed must-fix findings, direct evidence, any required visual QA evidence, any required `ux-review` or `devex-review` evidence, and accepted criteria satisfaction.
- Do not continue to the next requirement slice until the current slice has been clarified, reviewed, planned, passed any required `plan-design-review`, `plan-ux-review`, and `plan-devex-review`, implemented, verified, passed any required `visual-qa-hardening`, `ux-review`, and `devex-review`, passed `implementation-brake`, and completed `closeout`.
- After `closeout`, choose the next unchecked requirement from `sequence.md` and run the same gate order for that requirement; do not skip ahead unless the user explicitly changes the sequence.
...
- If a required gate, artifact, or acceptance criterion is missing or ambiguous, record the blocker and stop or clarify; do not invent a substitute path.
- Keep progress, decision, and evidence entries concise enough to survive long-running goal context compaction.
- If the sequence cannot be ordered without a product decision, ask the user instead of inventing the order.

## Anti-Patterns

- Treating a ready requirement as direct implementation permission.
- Treating words in the sequence id or title as the source of truth when the user's requested artifact class and the checklist's execution unit point to different sequences.
- Hiding a distinct reference, clone-coding, design, or input-contract deliverable as a prerequisite note inside a screen/module implementation sequence.
- Adding a related-looking requirement to an existing sequence without user approval.
- Running more than one requirement slice at a time without an explicit sequence reason.
- Hiding gate state in chat instead of durable state files.
- Turning orchestration into a giant state machine that duplicates specialist skill logic.
