---
name: closeout
description: Thin post-[SHIP] executor for closing an already approved implementation by summarizing evidence, syncing active context, running a default bounded green-refactor pass for code work, routing follow-ups in the final response, and creating a local commit without push, PR, merge, or full re-review.
---
...
### 4. Default bounded green-refactor pass

Classify the closeout by intended diff:

- Code work: execution code, tests, scripts, or runtime configuration changed.
- Non-code work: only docs, skills, policies, specs, or other guidance changed.

For code work, inspect the touched area for a small green refactor candidate before committing. The touched area is the files changed by the current task plus directly adjacent modules needed to understand those changes. Do not scan unrelated dirty files, repo-wide smells, or broad architecture moves during closeout.

For non-code work, code refactor scanning is not required. Record that the closeout is non-code and skip this step.
...
- no more than 220 changed lines for the closeout refactor patch

Do not change acceptance criteria, user-visible behavior, policy, data shape, or integration contracts during closeout.

For code work, record exactly one outcome:

- a safe, useful refactor was performed
- no safe or useful touched-area candidate was found
- a candidate exists but exceeds closeout scope, so closeout is blocked

Block with `[CLOSEOUT_BLOCKED]` only for ship-risk refactor gaps: issues that materially increase bug risk, test gaps, contract confusion, or likely rework. Do not block for style preferences, file length alone, naming taste, or broad cleanup that is not tied to ship risk.

If a useful candidate needs more than 220 changed lines, broader tests, a renewed `[SHIP]` judgment, ownership-boundary changes, or architectural redesign, do not perform it in closeout. Stop with `[CLOSEOUT_BLOCKED]` and route back to implementation plus `implementation-brake`.
