...
Before generating the checklist, confirm that the user's requested artifact class matches the checklist's execution unit. A reference-material contract, clone-coding contract, design/input contract, screen implementation sequence, route/module implementation sequence, data migration, and launch readiness sequence are separate sequence candidates unless the user explicitly asks to combine them.

Do not add, insert, or rewrite requirement paths in an existing sequence merely because a new request seems related, prerequisite-like, or useful for that sequence. Changing a sequence checklist changes sequence scope and requires explicit user approval unless the user directly asked to modify that sequence.
...
The outcome must name the artifact class this sequence produces; do not hide a distinct reference, clone-coding, design, or input-contract deliverable as a prerequisite note inside an implementation sequence.
