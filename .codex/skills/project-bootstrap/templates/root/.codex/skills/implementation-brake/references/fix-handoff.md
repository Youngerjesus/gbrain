...
`must fix now`이고 observable behavior 로 환원 가능한 finding 은 가능한 한 빨리 `tdd-workflow`의 다음 failing test 로 환원해야 합니다.
증거가 부족하거나 인간 결정이 필요한 finding 은 production patch 로 바로 넘기지 않습니다.
...
- `blocked on evidence` 또는 `blocked on human decision` 을 repair task 로 위장하지 않습니다.
