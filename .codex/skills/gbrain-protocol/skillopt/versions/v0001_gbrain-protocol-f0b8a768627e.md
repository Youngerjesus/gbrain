...
2. If the user supplied a source-root file path, derive the repo-like slug from
   that path and read it with `gbrain get <slug> --source <source-id>` before
   any broad summary.
...
5. Open and inspect enough top candidates with `gbrain get` or direct source
   file reads to confirm substance. Do not finalize a top result from title or
   snippet alone.
6. Return roughly the top 10 relevant pages by default. If fewer than 10 are
   actually relevant, say so; if more than 10 are strong matches, return the top
   10 plus a short note about additional candidates.
