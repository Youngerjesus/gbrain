...
The conformance reviewer is companion evidence only. The main `implementation-brake` owns final ship-readiness verdict and remains the only owner of `[SHIP]`, `[FIX BEFORE SHIP]`, or `[NEEDS REWORK]`.

Required conformance evidence before `[SHIP]`:

- structured `conformance_result_status`
- AC coverage summary that maps accepted requirements to implementation and verification evidence
- unresolved conformance findings and their reconciliation
- residual risk handling
- evidence reference consumed by implementation-brake

Allowed `conformance_result_status` values:

- `CONFORMANT`: implementation satisfies accepted requirements with sufficient AC coverage
- `FINDINGS`: material conformance findings require reconciliation
- `BLOCKED_INVALID`: conformance input or output is malformed, contradictory, or insufficient
- `BLOCKED_UNAVAILABLE`: reviewer or runtime unavailable

Unavailable policy:

- A missing conformance result, `BLOCKED_UNAVAILABLE`, or missing conformance result evidence cannot treat the implementation as conformant.
- Production/launch-bound work blocks `[SHIP]` when the conformance reviewer is unavailable; production/launch-bound unavailable conformance remains a ship blocker.
- Ordinary non-production work may use explicit `FALLBACK_SELF_REVIEW_USED` only when the fallback records AC coverage, residual risk, and why external reviewer evidence was unavailable.
