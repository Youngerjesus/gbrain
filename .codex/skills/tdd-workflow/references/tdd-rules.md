...
This is the gate for production behavior changes. For UI polish, documentation, configuration, verification scripts, formatting, generated metadata, or tiny changes already covered by existing tests, use the skill's pragmatic exception path and report concrete exception evidence instead of pretending a red/green cycle happened.
...
## Pragmatic Exception Boundary

The exception path is not a loophole for behavior changes. It is for bounded work where creating a new failing test would be artificial, duplicative, or less truthful than the existing verification evidence.

Allowed examples:

- UI polish constrained by screenshots, visual QA, existing component tests, or browser inspection.
- Documentation-only updates constrained by spelling, link, or render checks.
- Configuration-only changes constrained by parser, lint, or startup checks.
- Verification script cleanup constrained by running the script itself and a known failing fixture when relevant.
- Tiny code changes where an existing targeted test already covers the behavior and a new test would only duplicate it.

Required proof:

1. State why no new failing test was added.
2. Name the existing tests, checks, screenshots, fixtures, or inspections used as evidence.
3. Run the narrowest relevant verification after the edit.
4. Run the repo-required broader verification before completion.
