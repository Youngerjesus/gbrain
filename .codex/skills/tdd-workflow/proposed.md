...
1. State the target behavior and success condition in one sentence, and constrain it to exactly one observable behavior.
2. Convert the requirement into executable acceptance criteria before choosing the test shape, with a single-case narrowing statement:
   - `Given <specific preconditions/data state>`
   - `When <specific input/event>`
   - `Then <single observable result/state/error>`.
   - Include non-goals so unrelated behavior is explicitly out of scope.
...
2. Identify and name the exact existing tests, snapshots, checks, screenshots, or manual inspection that constrain the change (including command names or artifact paths).
...
- `Verification strategy`: deterministic, empirical, probabilistic, runtime, migration, or a justified mix.
...
- No production behavior code before a failing test unless a documented pragmatic exception applies. Do not claim TDD when red proof was not observed.
