...
When asked to validate a returned report, quote or restate each required field by its exact field name in the validation result, even when the supplied report is missing or unusable:
...
For missing fields, mark the named field `missing` or `not present`; do not substitute alternate labels such as candidates, tests, or findings only. This keeps validation output machine-checkable while still blocking editing on an incomplete report.
